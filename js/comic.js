(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
			...
			var closest_index = -1;
			var closest_dist = 442;
			for (var j = 0; j < palette.length; ++j)
			{
				let r_diff = rgb.r - palette[j][0];
				let g_diff = rgb.g - palette[j][1];
				let b_diff = rgb.b - palette[j][2];
				let distance = Math.hypot(r_diff, g_diff, b_diff);
			...
			outputData.data[i]     = palette[closest_index][0];
			outputData.data[i + 1] = palette[closest_index][1];
			outputData.data[i + 2] = palette[closest_index][2];
